            ...
            <div class="col-md-12 col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="mb-0 card-title">Edit Match</h3>
                    </div>
                    <div class="card-body">
                                ...
                                <div class="col-md-6">
                                        ...
                                        <select class="form-control select2 custom-select" name="first_team_id" data-placeholder="">
                                            ...
                                            @if(count($team_data) > 0)
                                                @foreach($team_data as $key => $value)
                                                    <option value="{{ $value->id }}" {{  ($value->id == old('first_team_id') ? 'selected' : ($match->first_team_id == $value->id ? 'selected' : ''))  }}>{{ $value->name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control select2 custom-select selectResult" name="result" data-placeholder="Choose one">
                                            ...
                                            <option value="0" {{ '0' == old('result') ? 'selected' : ($match->result == '0' ? 'selected' : '')  }}>Draw</option>
                                            <option value="1" {{ '1' == old('result') ? 'selected' : ($match->result == '1' ? 'selected' :'')  }}>Winner</option>
                                        </select>
                                        @if ($errors->has('result'))
                                            <span class=" text-danger">{{ $errors->first('result') }}</span>
                                        @endif
                                    </div>
                                    <div class="form-group chooseWinner" style="{{ old('result') == '0' ? 'display:none' : ($match->result == '0' ? 'display:none' : '') }}">
                                        <div class="form-label">Winner</div>
                                        <div class="custom-controls-stacked">
                                            <label class="custom-control custom-radio">
                                                <input type="radio" class="custom-control-input" name="winner_team_id" value="1" {{ old('winner_team_id') == '1' ? 'checked' : ($match->first_team_id == $match->winner_team_id ? 'checked' : '') }}>
                                                <span class="custom-control-label">First Team</span>
                                            </label>
                                            <label class="custom-control custom-radio">
                                                <input type="radio" class="custom-control-input" name="winner_team_id" value="2" {{ old('winner_team_id') == '2' ? 'checked' : ($match->second_team_id == $match->winner_team_id ? 'checked' : '') }} >
                                ...
                                <div class="col-md-6">
                                    <div class="form-group ">
                                        ...
                                        <select class="form-control select2 custom-select" name="second_team_id" data-placeholder="Choose one">
                                            ...
                                            @if(count($team_data) > 0)
                                                @foreach($team_data as $key => $value)
                                                    <option value="{{ $value->id }}" {{ $value->id == old('second_team_id') ? 'selected' : ($match->second_team_id == $value->id ? 'selected' : '')  }}>{{ $value->name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    ...
                                    <div class="form-group">
                                        <div class="wd-200 mg-b-30">
                                            ...
                                            <div class="input-group">
                                                ...
                                                </div><input name="scheduled_date" class="form-control fc-datepicker" placeholder="MM/DD/YYYY" type="text" value="{{ old('scheduled_date') ? old('scheduled_date') : (\Carbon\Carbon::parse($match->scheduled_date)->format('m/d/Y')) }}">
                                            </div>
